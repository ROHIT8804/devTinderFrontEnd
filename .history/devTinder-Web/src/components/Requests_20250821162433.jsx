...
const Requests = () => {
  ...
  const getrequestData = async () => {
    ...
    try {
      const response = await axios.get(BASE_URL + '/users/requests/recieved', { withCredentials: true });
      ...
      if (error.response && error.response.status === 401) {
        ...
        setNoRequestsMessage(error.response.data || "No connection requests received");
    ...
    <>
      <div className=" bg-gray-100 p-3 rounded-2xl">
        {shouldShowNoDataCard() ? (
          ...
          {Array.isArray(requestData) && requestData.map((requestData) => (
            <div
              key={requestData._id}
            ...
            >
              <div className="relative">
                <img
                  src={requestData?.fromUserId?.photoUrl || "https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=400&h=400&fit=crop&crop=face"}
              ...
              <div className="p-6">
                <h2 className="text-2xl font-bold text-white mb-3 text-center">
                  {requestData?.fromUserId?.firstName} {requestData?.fromUserId?.lastName}
                </h2>
